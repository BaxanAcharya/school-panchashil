...
export const verifyJWT = handleAsync(async (req, _, next) => {
  try {
    ...
    if (!token) throw new GenericError(401, "NOT AUTHORIZED");
    const decodedToken = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET);
    const loggedAdmin = Admin.findById(decodedToken?._id).select(
      `-${PASSWORD} -${REFRESH_TOKEN}`
    );
    if (!loggedAdmin) throw new GenericError(401, "Invalid access token");
    req.admin = decodedToken?._id;
    next();
  } catch (error) {
    console.log(error);
    throw new GenericError(401, error?.message || "Invalid access token");
  }
});
